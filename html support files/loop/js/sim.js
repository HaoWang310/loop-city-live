...
(function () {
  ...
  function World(cfg) {
    ...
    // The nodes. A field for each would be 15 MB apiece on the whole loop, so only the nearest-node field
    // is kept; a node's own distance to a cell is two multiplies, worked out where it is wanted.
    this.nodes = cfg.nodes.map(function (nd) {
      return { id: nd.id, t: nd.t, zone: nd.zone, alloc: nd.alloc,
               cx: (nd.x - cfg.x0) / pxPerCell, cy: (nd.y - cfg.y0) / pxPerCell };
    });
    var DN = new Float32Array(N), nds = this.nodes, q2, dx, dy, d2, best, k2;
    for (r = 0; r < h; r++) for (c = 0; c < w; c++) {
      q2 = r * w + c; best = Infinity;
      for (k2 = 0; k2 < nds.length; k2++) {
        if (!nds[k2].alloc) continue;
        dx = c + 0.5 - nds[k2].cx; dy = r + 0.5 - nds[k2].cy; d2 = dx * dx + dy * dy;
        if (d2 < best) best = d2;
      }
      DN[q2] = Math.sqrt(best) * C;
    }
    this.DNEAR = DN;
  ...
  function run(world, Pin) {
    ...
    world.nodes.forEach(function (nd) {
      ...
      var prim = nd.t === "P";
      streams.push({ sid: sid++, kind: "node", node: nd, zone: nd.zone,
    ...
    // The cells each stream may draw from, nearest first (nodes) or along the line (ribbon). Each
    // candidate's distance is kept beside it in the same order, so no field over the whole grid is needed.
    streams.forEach(function (st) {
      var list = [], dl = [], q, r, c, dx, dy, d, j;
      if (st.kind === "node") {
        var cx = st.node.cx, cy = st.node.cy, rc = st.maskR / C, rad2 = rc * rc;
        var r0 = Math.max(0, Math.floor(cy - rc - 1)), r1 = Math.min(h - 1, Math.ceil(cy + rc + 1));
        var c0 = Math.max(0, Math.floor(cx - rc - 1)), c1 = Math.min(w - 1, Math.ceil(cx + rc + 1));
        for (r = r0; r <= r1; r++) for (c = c0; c <= c1; c++) {
          q = r * w + c;
          if (!build[q] || world.zone[q] !== st.zone) continue;
          dx = c + 0.5 - cx; dy = r + 0.5 - cy; d = dx * dx + dy * dy;
          if (d > rad2) continue;
          list.push(q); dl.push(Math.sqrt(d) * C);
        }
      } else {
        for (q = 0; q < N; q++) {
          if (!build[q] || DRAIL[q] > 2600 || world.zone[q] !== st.zone) continue;
          list.push(q); dl.push(DRAIL[q]);
        }
      }
      var idx = new Int32Array(list.length);
      for (j = 0; j < idx.length; j++) idx[j] = j;
      Array.prototype.sort.call(idx, function (a, b) { return dl[a] - dl[b]; });
      st.order = new Int32Array(idx.length);
      st.dist = new Float32Array(idx.length);
      for (j = 0; j < idx.length; j++) { st.order[j] = list[idx[j]]; st.dist[j] = dl[idx[j]]; }
      st.list = new Int32Array(0); st.n = 0;                 // the cells this stream has built
    });

    // desirability, as the sandbox's fields(): the node and the line as magnets, times vein, times the field
    var LU = new Float32Array(N), DJIT = new Float32Array(N), DNEAR = world.DNEAR;
    for (i = 0; i < N; i++) {
    ...
    function build1(q, st, y) {
      ...
      if (st.n >= st.list.length) {                          // the stream keeps its own cells, for promotion
        var g = new Int32Array(Math.max(4096, st.list.length * 2));
        g.set(st.list); st.list = g;
      }
      st.list[st.n++] = q;
    ...
    function growthYear(y) {
      ...
      for (var s = 0; s < streams.length; s++) {
        ...
        } else { rad = 2600; wide = 2600; }
        for (j = 0; j < st.order.length; j++) {
          q = st.order[j];
          if (st.dist[j] > rad) {
            if (open >= need || rad === wide) break;
            rad = wide;
            if (st.dist[j] > rad) break;
          }
          if (tier[q] !== 0) continue;
          open++;
    ...
    function promotionYear(y) {
      var densReady = false;
      for (var s = 0; s < streams.length; s++) {
        var st = streams[s], k, q, haveMH = 0, haveH = 0, L = st.list, n = st.n;
        for (k = 0; k < n; k++) { q = L[k]; if (tier[q] >= 2) haveMH++; if (tier[q] === 3) haveH++; }
        var wantMH = roundHE((st.M + st.H) * ramp(y, st.mStart, 0.85));
        if (wantMH > haveMH) {
          heap.reset(wantMH - haveMH);
          if (st.kind === "node") {
            for (k = 0; k < n; k++) { q = L[k]; if (tier[q] === 1 && DJIT[q] >= P.rcore) heap.push(-DJIT[q], q); }
          } else {
            if (!densReady) { flushD(); densReady = true; }
            for (k = 0; k < n; k++) { q = L[k]; if (tier[q] === 1) heap.push(DENS[q] * (0.4 + 1.2 * V[q]), q); }
          }
        ...
        if (st.hStart !== null && st.H > 0) {
          ...
          if (wantH > haveH) {
            ...
            for (k = 0; k < n; k++) { q = L[k]; if (tier[q] === 2 && DJIT[q] >= P.rmed) heap.push(-DJIT[q], q); }
    ...
    var LWV = null, LWH = null, farmCand = null;
    function farmWeights(DFIN) {
      ...
      for (var q = 0; q < N; q++) {
        ...
        if (LWV[q] > -1e8 || LWH[q] > -1e8) cand.push(q);
      }
      farmCand = Int32Array.from(cand);
    }
    function farmingYear(y) {
      ...
      for (var t = 0; t < 2; t++) {
        ...
        if (need <= 0) continue;
        var LW = tp[0], base = drawBase(P.seed, tp[4], y), q, ci;
        heap.reset(need);
        for (ci = 0; ci < farmCand.length; ci++) {
          q = farmCand[ci];
          if (tier[q] !== 0 || LW[q] < -1e8) continue;
          heap.push(LW[q] + gum(base, q), q);
        }
        for (j = 0; j < heap.cnt; j++) { q = heap.idx[j]; tier[q] = tp[1]; yFarm[q] = y; cnt[tp[3]]++; }
      }
    }
    ...
    function takeSnapshot() {
      snap = { tier: tier.slice(), own: own.slice(), B: B.slice(), yB: yB.slice(), yM: yM.slice(), yH: yH.slice(),
        ...
        cnt: JSON.parse(JSON.stringify(cnt)), got: streams.map(function (st) { return st.got; }),
        lists: streams.map(function (st) { return st.list.slice(0, st.n); }), stats: stats.slice() };
    }
    function beginPass2() {
      ...
      streams.forEach(function (st, s) {
        st.got = snap.got[s]; st.list = snap.lists[s].slice(); st.n = st.list.length;
      });
