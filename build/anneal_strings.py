# -*- coding: utf-8 -*-
"""List every piece of Chinese text in the teammate's ANNEALING LAB that a person can see, for translation.

    "C:/Users/Shreya/AppData/Local/Python/bin/python.exe" anneal_strings.py

Reads cell/anneal/anneal_zh_original.html (his annealing lab, byte for byte as he sent it) and writes
build/anneal_strings.json: one record per distinct literal -- the exact source text of a JS string (quotes
...
The translation is applied by build/anneal_translate.py from a mapping of these exact literals, so a literal
...
OUT = os.path.join(HERE, "anneal_strings.json")
